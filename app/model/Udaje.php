...
class Udaje extends Table {
   ...
   * @return array */
  public function hlavneUdaje($language = "sk") {
    $vysledok = [];
    //Nacitanie len tych premennych, ktore platia pre danu jazykovu mutaciu
    foreach ($this->findAll()->fetchPairs('nazov', 'text') as $key => $value) { 
      $kluc = explode("-", $key);
      if (count($kluc) == 2 && $kluc[1] == $language) { $vysledok[substr($key, 0, strlen($key)-strlen($language)-1)] = $value; } 
