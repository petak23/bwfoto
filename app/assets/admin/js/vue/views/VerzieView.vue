...
<div class="col-12">
  ...
  <div class="row">
    <div class="col-12">
      <div class="accordion" id="accordionVerzie" v-for="ve in items" :key="ve.id">
        <div class="card {first}bg-success{/first}" >
          <div class="card-header p-0 text-white" :id="'heading'+ve.id">
            <h5 class="mb-0">
              <button class="btn {if $iterator->first}btn-success{else}btn-light{/if}" type="button" 
								data-toggle="collapse" data-target="#collapse{$ve->id}" aria-expanded="true" aria-controls="collapse{$ve->id}">
