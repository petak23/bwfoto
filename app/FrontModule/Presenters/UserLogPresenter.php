...
class UserLogPresenter extends BasePresenter
{
  ...
  public function actionDefault()
  {
    ...
    $this["userEditForm"]->setDefaults([
      'avatar' => $this->uzivatel->user_profile->avatar,
  ...
  protected function createComponentMailChangeForm(): Form
  {
    $form = new Form();
    $form->addProtection();
  ...
  public function actionActivateNewEmail($id, $new_email_key)
  {
    ...
    if ($new_email_key == $user_main_data->new_email_key) { //Aktivacia prebeha v poriadku
      try {
        $this->user_main->uloz([
          'email' => $user_main_data->new_email,
          'new_email' => NULL,
          'new_email_key' => NULL
        ], $user_main_data->id);
      ...
      $this->flashMessage($this->texty_presentera->translate('activate_mail_err1'), 'danger');
    }   //Neuspesna aktivacia
