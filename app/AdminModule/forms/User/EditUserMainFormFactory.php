...
class EditUserMainFormFactory
{
	...
	public function create(array $user_view_fields): Form
	{
		...
		$form->setCurrentGroup(NULL);
		if ($user_view_fields["categori"]) {
			$form->addCheckboxList('categori', 'Kategória', $this->user_categories->findAll()/*findBy(["main_category != 'B'"])*/->fetchPairs('id', 'name'));
		}
