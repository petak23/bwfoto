...
class AddUserFormFactory
{
	...
	public function create(): Form
	{
		$form = new Form();
		$form->addProtection();
		$form->addText('name', 'Meno a PRIEZVISKO', 80, 80)
			->addRule(Form::MIN_LENGTH, 'Meno a priezvisko musí mať aspoň %s znakov', 2)
	...
	public function addUserFormSubmitted(Nette\Forms\Controls\SubmitButton $button)
	{
		$values = $button->getForm()->getValues();
		try {
			$this->user_main->add($values->name, $values->email, $values->heslo, 1, $values->id_user_roles);
